...
export function formatarDataHora(dataStrInput, horaStrInput, returnObject = false) {
    ...
    // Formatação respeitando fuso horário de Cuiabá (America/Cuiaba)
    const dateFormatter = new Intl.DateTimeFormat('pt-BR', { timeZone: 'America/Cuiaba', day: '2-digit', month: '2-digit', year: 'numeric' });
    const timeFormatter = new Intl.DateTimeFormat('pt-BR', { timeZone: 'America/Cuiaba', hour: '2-digit', minute: '2-digit', hour12: false });
    const finalDate = dateFormatter.format(dataHoraInstance);
    const finalTime = timeFormatter.format(dataHoraInstance);
