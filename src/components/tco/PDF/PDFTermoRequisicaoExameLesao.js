...
const formatarDataPorExtenso = (date) => {
    try {
        // Usa o fuso horário de Cuiabá e extrai partes da data
        ...
        const mes = (parts.find(p => p.type === 'month')?.value || '').toUpperCase();
        const ano = parts.find(p => p.type === 'year')?.value || '';
        if (!dia || !mes || !ano) throw new Error('Partes de data inválidas');
